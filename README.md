...
## Full-Image Runs (`complete_img/`)

`complete_img/image1` … `complete_img/image5` hold full-frame (no `--roi`)
extractions of `image1.jpg` and `image2.png`–`image5.png`. Without an ROI the
detector also sees smoke-tube pillars, window-blind reflections, and wall
texture, so the length filters are raised: real smoke lines are long and
continuous, clutter is short.

For `image1.jpg` (1706 px wide):
...
For `image2.png`–`image5.png` (~2900 px wide, thresholds scaled ~1.7x):
...
The video pipeline (`analyze_video_streamlines.py`, driven by
`scripts/run_all_videos.sh`) shares this detection code via
`detect_streamline_polylines()`, so per-frame annotation uses the same peak
tracer and contrast normalization automatically. The script annotates the
full x range while keeping the old vertical crop (`--roi 0,120,1920,760`)
with `--ridge-threshold 7 --min-length 180 --min-horizontal-span 120
--horizontal-ratio 2`; these filters keep the long continuous smoke lines
(~115 per frame) while dropping pillar speckle and glass-reflection clutter.
Existing `video_output_*` folders were generated by the legacy skeleton
tracer on a right-side-only ROI; rerun with `FORCE=1
scripts/run_all_videos.sh` to regenerate them.
